...
describe Pet, type: :model do
  ...
  describe "instance methods" do
    ...
    describe "#pending_application?" do
      it 'find if pet has any pending applications' do
        shelter = create(:shelter, id: 1)
        @pet1 = create(:pet, id: 1, shelter_id: 1)
        @application = create(:application, id: 1, status: :pending)
        @application2 = create(:application, id: 2, status: :approved)
        ...
        expect(@pet1.pending_application?).to eq(true)
      end
    end
    describe "#adoption_status" do
      describe 'find if pet has an adoption status' do
        it "is Adopted" do
          pet = create(:pet, adoptable: false)
          expect(pet.adoption_status).to eq ("Adopted")
        end
        it "is Not Adopted" do
          pet = create(:pet, adoptable: true)
          expect(pet.adoption_status).to eq ("Not Adopted")
        end
      end
    end
  end
